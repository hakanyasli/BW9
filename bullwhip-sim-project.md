...
| **v1.0** | *(Fable session)* **Bugs fixed:** PDF export volatility thresholds were still on the old 1–8 scale, so every report said "Extreme" — now uses km/h cutoffs matching `showChart()`. Export delay→days translation aligned with the chart modal (`(dly-1)*3` days). Hardcoded English "WHEELS (delayed)" label moved into the i18n system (`wheelsLbl`, via `data-lbl` attribute).<br>**Features:** (1) **Ghost-run overlay** — the previous run's action line is drawn dashed on the analytical chart for direct A/B delay comparison (`prevRun` state, guarded by runId). (2) **Upstream Amplification Cascade** — consumer→retailer→wholesaler→factory bars compounding the measured ratio per tier (log-scaled bars). (3) **Knowledge-check quiz** — 3 bilingual questions with instant feedback and explanations, rendered in the chart modal, delegated click handling. (4) **PWA** — `manifest.json` + `sw.js` (network-first caching, offline-capable) + `icon.svg`; SW registered at boot. Version bumped across all display text. |
